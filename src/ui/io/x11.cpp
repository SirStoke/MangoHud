// Originally from: https://github.com/ocornut/imgui/pull/7262
...
static ImGui_ImplXlib_Data *ImGui_ImplXlib_GetBackendData() {
  return ImGui::GetCurrentContext()
             ? (ImGui_ImplXlib_Data *)ImGui::GetIO().BackendPlatformUserData
             : nullptr;
}
...
static const char *ImGui_ImplXlib_GetClipboardText(void *) {
  ...
  if (owner == None) {
    ...
    owner = bd->Win;
    XConvertSelection(bd->Dpy, bd->XA_CLIPBOARD, bd->XA_MIME[1],
                      bd->XA_SELECTION, owner, CurrentTime);
    XSync(bd->Dpy, 0);
    ...
    if (XGetWindowProperty(bd->Dpy, owner, bd->XA_SELECTION, 0, INT_MAX / 4,
                           False, bd->XA_MIME[1], &seln_type, &seln_format,
                           &count, &overflow, &src) == Success) {
...
// your application based on those two flags. If you have multiple events and
// some of them are not meant to be used by dear imgui, you may need to filter
...
bool ImGui_ImplXlib_ProcessEvent(XEvent *event) {
  ...
  case GenericEvent: {
    ...
    if (cookie->extension == bd->Xi2Opcode &&
        XGetEventData(event->xcookie.display, cookie)) {
      ...
      case XI_ButtonRelease: {
        if (dev->detail >= Button1 && dev->detail <= Button3) {
          int mouse_button = -1;
          if (dev->detail == Button1) {
            mouse_button = 0;
          }
          if (dev->detail == Button2) {
            mouse_button = 1;
          }
          if (dev->detail == Button3) {
            mouse_button = 2;
          }
  ...
  case KeyRelease: {
    ...
    if (bd->IC && event->type == KeyPress) {
      int size = Xutf8LookupString(bd->IC, &event->xkey, text, sizeof(text),
                                   NULL, &status);
  ...
  case SelectionRequest: {
    ...
    if (req->target == bd->XA_TARGETS) {
      XChangeProperty(bd->Dpy, req->requestor, req->property, 4 /* XA_ATOM */,
                      32, PropModeReplace, (unsigned char *)bd->XA_MIME, 6);
      sevent.xselection.property = req->property;
      sevent.xselection.target = bd->XA_TARGETS;
    } else if (bd->ClipboardTextData) {
      for (unsigned int i = 0;
           i < sizeof(text_mime_types) / sizeof(text_mime_types[0]); i++) {
        if (req->target != bd->XA_MIME[i])
          continue;

        XChangeProperty(bd->Dpy, req->requestor, req->property, req->target, 8,
                        PropModeReplace, (unsigned char *)bd->ClipboardTextData,
                        strlen(bd->ClipboardTextData));
        sevent.xselection.property = req->property;
...
static void ImGui_ImplXlib_UpdateMouseData() {
  ...
  if (is_app_focused) {
    ...
    if (io.WantSetMousePos)
      XWarpPointer(bd->Dpy, None, bd->Win, 0, 0, 0, 0, (int)io.MousePos.x,
                   (int)io.MousePos.y);

    // (Optional) Fallback to provide mouse position when focused (XI_Motion
    // already provides this when hovered or captured) if (bd->MouseButtonsDown
    // == 0)
    // {
    //     int window_x, window_y, mouse_x_global, mouse_y_global;
    //     Window root, child;
    //     unsigned int mask;
    //     if (True == XQueryPointer(bd->Dpy, bd->Win, &root, &child,
    //     &mouse_x_global, &mouse_y_global, &window_x, &window_y, &mask))
...
static void ImGui_ImplXlib_UpdateMouseCursor() {
  ...
  if (io.MouseDrawCursor || imgui_cursor == ImGuiMouseCursor_None) {
    ...
    Cursor expected_cursor = bd->MouseCursors[imgui_cursor]
...
void ImGui_ImplXlib_NewFrame() {
  ...
  if ((current_time.tv_sec < bd->Time.tv_sec) ||
      ((current_time.tv_sec == bd->Time.tv_sec) &&
       (current_time.tv_nsec < bd->Time.tv_nsec))) {
    current_time.tv_sec = bd->Time.tv_sec;
    current_time.tv_nsec = bd->Time.tv_nsec + 1;
  }

  if (bd->Time.tv_sec > 0 || bd->Time.tv_nsec > 0)
    io.DeltaTime = (float)(current_time.tv_sec - bd->Time.tv_sec) +
                   (float)((double)(current_time.tv_nsec - bd->Time.tv_nsec) /
                           1000000000.0f);
  else
...
static long get_window_pid(Display *dpy, Window w) {
  ...
  if (XGetWindowProperty(dpy, w, pid_atom, 0, 1, False, XA_CARDINAL,
                         ...
                         &prop) == Success) {
    if (prop && nitems >= 1) {
      // XA_CARDINAL usually returns 32-bit per item
      pid = *(long *)prop;
    }
  }
  if (prop)
...
static int is_transient(Display *dpy, Window w) {
  Window trans = None;
...
static Window top_level_ancestor(Display *dpy, Window root, Window w) {
  Window current = w, parent, root_return;
  Window *children;
  unsigned int nchildren;
  // climb until parent == root or can't climb
  while (1) {
    if (XQueryTree(dpy, current, &root_return, &parent, &children,
                   &nchildren) == 0)
      break;
    if (children)
      XFree(children);
...
Window find_window(Display *dpy) {
  if (dpy == NULL) {
    SPDLOG_ERROR("Invalid display, cannot find process window");
  ...
  for (unsigned int i = 0; i < nchildren; ++i) {
    ...
    SPDLOG_INFO("Checking window {0:d}, pid {1:d}", w, win_pid);

    if (win_pid != 0 && (pid_t)win_pid == pid) {
      SPDLOG_INFO("Found window {0:d}", w);

      // Check visibility and transientness
      if (is_transient(dpy, w))
        continue; // skip dialog windows
